...
| Per-bar Poly override "missing arms" on chart-visible streaks — when Poly resolved tiny-move bars opposite to Binance (e.g., 2026-05-10 13:30 UTC: 6-bar UP per chart, Poly disagreed at 13:10 +0.014%, bot saw streak=3 → no arm). User expectation is arm tracks chart visual. | TBD — revert streak detection to Binance close-vs-open only; outcome at T-0 still uses Poly midpoint (Bug 1 preserved). Mismatch alerts inform user when chart bet may resolve against Poly truth. |

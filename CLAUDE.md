...
| `fetchStreakWithVolume` suppressed streak to 0 on ANY Binance/Poly mismatch — disabled arming for real Poly streaks (verified prod 2026-05-07 02:00-02:30: 5-bar DOWN streak per Poly, 1 mismatch at 01:55, bot saw streak=0 → never armed despite extreme streak running) | TBD — use Poly outcomes as ground truth (Binance fallback only when Poly NULL) instead of suppressing on mismatch |

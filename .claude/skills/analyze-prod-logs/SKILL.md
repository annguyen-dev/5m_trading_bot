...
| `echo state restored from cache` (info) — should fire ONCE per coin per worker start | Confirms post-restart restore working. If missing: echo_state_cache table empty (first deploy) OR DB unreachable at startup. Cross-check with `echo backfill .* done` log: backfill `lastEchoTriggerAt` should be ≤ restored value when persisted state is fresher. |
| `chainActive:true` in echo state log + `CHAIN — N arms in Mm` in adaptive threshold reason | Chain soft-defensive engaged. Threshold bumped (signal +chain_signal_bump, baseline +chain_baseline_bump). Verify configured: `echo_chain_enabled`. If active too often: lookback too long or threshold too low. If never active during known chain regime: enable cfg or lower threshold. |

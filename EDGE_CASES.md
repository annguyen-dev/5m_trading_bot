...
|---|---|---|
| **BTC 5m** | s3 ratio≥1.2 · s6 ratio≥1.0 · s7 ratio≥1.0 | POLY_1271, chung |
| **BTC 1h** | s3 ratio≥0.8 · s4 ratio≥1.0 | (chung) |
...
3. *(đề xuất, chưa code)* **Prior-clustering**: có ≥`priorCountMin` streak-peak cùng chiều ≥`priorStreakMin` trong `priorWindowMin` phút trước. Xem [§ Clustering](#clustering-edges).
...
### Clustering (ĐỀ XUẤT — chưa code) {#clustering-edges}
...
### Magnitude / over-extension — "đã tăng bao nhiêu" (ĐỀ XUẤT — chưa code) {#magnitude-edges}
